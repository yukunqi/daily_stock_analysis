...
### Added
- **TrendRadar article-content enrichment** — matched TrendRadar URLs can now be fetched with `newspaper3k` plus a BeautifulSoup fallback, cached in SQLite, and injected as article excerpts into stock-analysis prompts while falling back to title/link context on fetch failure.
- **TrendRadar passive news integration** — add local SQLite reader for TrendRadar `output/news/YYYY-MM-DD.db`, deterministic watchlist matching, TrendRadar stock-news context injection, and market-review news context without active news-search fallback.
- **Finance news live integration tests** — `tests/test_finance_news_live.py` adds opt-in real data-source validation gated by `RUN_FINANCE_NEWS_LIVE=1`, covering EastmoneyNews stock news/default `NOTICE,REPORT` filtering, RSSHub finance route fetches, `SearchService.search_comprehensive_intel()` against a real A-share symbol, and market-news dedup regression coverage.
- **Free finance news providers** — optional `EastmoneyNews` semantic finance-news search (`EASTMONEY_NEWS_ENABLED`, `EASTMONEY_NEWS_API_KEY`; defaults to excluding `NOTICE,REPORT`) and self-hosted `RSSHubFinance` feed search (`RSSHUB_FINANCE_ENABLED`, `RSSHUB_BASE_URL`, `RSSHUB_FINANCE_ROUTES`) now plug into the existing `SearchService` for stock news and market-review intelligence.
...
- **iWencai market data and intelligence (hithink-market-query)** — optional `IwencaiMarketQueryFetcher`: when `IWENCAI_MARKET_QUERY_ENABLED=true`, `IWENCAI_API_KEY` is set, and the skill CLI exists under `skills/hithink-market-query/`, Tonghuashun iWencai is preferred for A-share daily OHLCV, realtime quote fields (including volume ratio, turnover, PE/PB, market cap), chip distribution, stock base info, CN market indices, market breadth, sector rankings, and structured news/intelligence search. Add token `iwencai_market` or `wencai_skillhub` to `REALTIME_SOURCE_PRIORITY` for realtime source ordering. E2E helper: `scripts/verify_iwencai_pipeline.sh`.
