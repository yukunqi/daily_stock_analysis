...
"""
    ...
    def _build_fallback_market_news(self, overview: Optional[MarketOverview]) -> List[Dict[str, str]]:
        ...
        index_snapshot = (
            "；".join([f"{idx.name}{idx.change_pct:+.2f}%" for idx in overview.indices[:3] if idx.name])
            or "主要指数数据暂缺"
        )
    ...
    def _build_sector_block(self, overview: MarketOverview) -> str:
        ...
        if overview.bottom_sectors:
            ...
            for rank, sector in enumerate(overview.bottom_sectors[:5], 1):
                lines.append(f"| {rank} | {sector.get('name', '-')} | {self._format_signed_pct(sector.get('change_pct'))} |"
                )
        return "\n".join(lines)
